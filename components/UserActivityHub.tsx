      ...
      <div className="lg:col-span-8 space-y-6">
        <div className="flex items-center justify-between px-4">
           <div className="flex items-center gap-4">
              ...
              <div>
                 <h3 className="text-xl font-black text-white uppercase tracking-tighter font-tactical">Asset Acquisitions</h3>
                 <p className="text-[8px] text-neutral-600 font-tactical uppercase tracking-[0.4em]">Verified Handshakes // {userPurchases.length} Units</p>
              </div>
           </div>
           <div className="hidden sm:flex items-center gap-6 text-[9px] font-tactical font-black text-neutral-700 uppercase tracking-widest">
        ...
        <div className="space-y-4">
          {userPurchases.length > 0 ? (
                ...
                <div key={purchase.id} className="group relative bg-[#080808] border-2 border-neutral-900 rounded-[2rem] p-6 flex flex-col md:flex-row items-center justify-between gap-6 hover:border-[#FACC15]/30 transition-all shadow-xl overflow-hidden">
                   <div className="flex items-center gap-6 flex-1 w-full">
                      ...
                      <div className="min-w-0">
                         <div className="flex items-center gap-3 mb-1.5">
                            <span className="text-[8px] font-tactical font-black text-[#FACC15] uppercase tracking-[0.3em]">{purchase.purchaseMode === 'buy_now' ? 'INSTANT_SETTLEMENT' : 'AUCTION_WIN'}</span>
                            <span className="text-[7px] font-mono text-neutral-700 uppercase tracking-widest">ID: {purchase.id.slice(0, 10)}</span>
                         </div>
                         <h4 className="text-lg font-black text-white truncate uppercase tracking-tight leading-none group-hover:text-white transition-colors font-tactical">{purchase.leadTitle}</h4>
                         <div className="flex items-center gap-4 mt-2">
                            <span className="text-[9px] font-tactical font-black text-neutral-600 uppercase tracking-widest flex items-center gap-1.5">
                               <Clock size={10} /> {new Date(purchase.timestamp).toLocaleDateString()}
                            </span>
                            <div className="w-1 h-1 bg-neutral-800 rounded-full" />
                            <span className="text-[9px] font-tactical font-black text-neutral-600 uppercase tracking-widest">{originalLead?.category || 'General'} Sector</span>
                         </div>
                      </div>
                   </div>
                   ...
                   <div className="flex items-center gap-8 w-full md:w-auto justify-between md:justify-end border-t md:border-t-0 md:border-l border-neutral-900 pt-4 md:pt-0 md:pl-8">
                      <div className="text-left md:text-right shrink-0">
                         <span className="text-[8px] font-tactical font-black text-neutral-700 uppercase tracking-widest block mb-1">UNIT_COST</span>
            ...
            <div className="py-24 text-center bg-[#050505] border-2 border-neutral-900/40 border-dashed rounded-[3rem]">
               ...
               <h4 className="text-neutral-700 font-tactical text-xl uppercase tracking-[0.5em]">Inventory Empty</h4>
               <p className="text-neutral-800 text-[10px] font-tactical font-black uppercase tracking-widest mt-4">No assets have been provisioned to your ID yet</p>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="lg:col-span-4 h-full">
         <div className="bg-black p-8 rounded-[3rem] border border-neutral-800/60 h-full flex flex-col shadow-2xl relative overflow-hidden group">
            <div className="flex justify-between items-center border-b border-neutral-900 pb-6 mb-8 relative z-10">
               <h4 className="text-[11px] font-tactical font-black text-neutral-500 uppercase tracking-[0.4em] flex items-center gap-3">
            ...
            <div className="flex-1 space-y-6 overflow-y-auto scrollbar-hide relative z-10 max-h-[800px] pr-2">
               {userLogs.length > 0 ? (
                   ...
                   <div key={idx} className="relative pl-8 border-l border-neutral-900 pb-2">
                      ...
                      <div className="space-y-2">
                         <div className="flex justify-between items-center">
                            <span className="text-[8px] font-tactical font-black text-neutral-700 uppercase tracking-widest">{log.timestamp}</span>
                            <span className={`px-2 py-0.5 rounded text-[7px] font-tactical font-black uppercase tracking-tighter ${
                               log.type === 'buy' ? 'bg-emerald-600 text-white' : 'bg-neutral-800 text-neutral-400'
                            }`}>
                               {log.type}
                            </span>
                         </div>
                         <p className="text-[11px] text-neutral-400 font-bold uppercase leading-relaxed group-hover:text-white transition-colors font-tactical">
                            {log.message}
                         </p>
                         <div className="flex items-center gap-2 text-[8px] font-tactical font-black text-neutral-800 uppercase tracking-widest">
                 ...
                 <div className="py-24 text-center opacity-20">
                    ...
                    <p className="text-[9px] text-neutral-700 font-tactical font-black uppercase tracking-[0.5em]">Log Offline</p>
                 </div>
               )}
            </div>
            ...
            <div className="mt-8 pt-6 border-t border-neutral-900 relative z-10">
               <div className="bg-neutral-900/30 p-4 rounded-2xl border border-white/5 space-y-3">
                  <div className="flex justify-between items-center text-[9px] font-tactical font-black uppercase tracking-widest">
