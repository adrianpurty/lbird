...
const MobileNav: React.FC<MobileNavProps> = ({ activeTab, onTabChange, role }) => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-black/95 backdrop-blur-xl border-t border-[#1A1A1A] px-4 py-3 flex justify-between items-center z-50">
      {visibleItems.map((item) => (
        <button
          key={item.id}
          onClick={() => onTabChange(item.id)}
          className={`flex flex-col items-center gap-1 transition-all duration-300 min-w-[50px] ${
            activeTab === item.id ? 'text-[#FACC15]' : 'text-neutral-600'
          }`}
        >
